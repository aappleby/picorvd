...
int main() {
  ...
  while(1) {
    ...
    if (strcmp(command, "write_page") == 0) {
      on_write_page(sl, 0x08000000, (uint32_t*)blink_bin, blink_bin_len / 4);
    }
    if (strcmp(command, "unhalt") == 0) {
      sl.put(ADDR_DMCONTROL, 0x00000003);
      while(!sl.get_dmstatus().ANYHAVERESET); 
      sl.put(ADDR_DMCONTROL, 0x50000001);
      while (sl.get_dmstatus().ANYHALTED);
